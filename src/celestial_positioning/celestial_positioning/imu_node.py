...
class ImuNode(Node):
    ...
    def _read_and_publish(self):
        ...
        # Convert to physical units (sensor scale only — no calibration
        # correction applied; raw values flow through to the log)
        accel = np.array([ax, ay, az], dtype=np.float64) * ACCEL_SCALE_4G
        gyro = (np.array([gx, gy, gz], dtype=np.float64)
                * GYRO_SCALE_500DPS * DEG_TO_RAD)
        ...
        if self.mag_available:
            try:
                ...
                # Convert to Tesla (scale only — raw values, no calibration)
                mag = (np.array([mx, my, mz], dtype=np.float64)
                       * MAG_SCALE * GAUSS_TO_TESLA)
